...
uses
  ...
  ZnMD5, ULibFun, USysDB, UMITConst, UMgrChannel, DateUtils,superobject,
...
var nStr, nType: string;
    nNode: TXmlNode;
    nValue,nMoney, nFreezeMoney: Double;
begin
  Result := False;
  BuildDefaultXML;
  nMoney := 0 ;
  ...
  begin
    ...
    begin
      GetRemoteCusMoney(FIn.FData,nMoney);
      nMoney := nMoney - nFreezeMoney;
    end
    else
  ...
  nStr := 'select D_ZID,' +                     //销售卡片编号
        '  D_Type,Z_OnlyMoney,Z_FixedMoney,' +  //类型(袋,散)
        '  D_StockNo,' +                        //水泥编号
        '  D_StockName,' +                      //水泥名称
        '  D_Price,' +                          //单价
        '  D_Value,' +                          //订单量
    ...
    begin
      ...
      begin
        ...
        try
          ...
          begin
            if FieldByName('Z_FixedMoney').AsFloat > nMoney then
...
function TBusWorkerBusinessWebchat.GetRemoteCusMoney(var nCode: string;
  var nMoney: Double): Boolean;
var
