...
var nStr,nCode,nPrint: string;
  ...
  nArea, nA1: string;
const
  AdrAry: array[0..21] of string=('郑','州','洛','阳','三','门','峡','平','陆','潼',
                                  '关','卢','氏','豫','灵','宝','观','音','堂','宁',
                                  '渭','南');
begin
  Result := True;
  ...
  begin
    ...
    begin
      ...
      begin
        nA1 := Copy(nArea,i*2+1,2);
        for j := 0 to 21 do
        begin
          if AdrAry[j] = nA1 then
            nCode := nCode + '@R'+  IntToHex(J,2);
        end;
      end;
      nCode := nCode + '@7' + Trim(Fields[1].AsString); //批次号

      nA1 := Trim(Fields[0].AsString);
      nA1 := Copy(nA1, Length(nA1)-2, 3);  //交货单后单位
      nA1 := Date2Str(FieldByName('L_InTime').AsDateTime, False) + nA1;   //日期+交货单后三位
