...
implementation
uses
  ULibFun, UBusinessPacker, USysLoger, UBusinessConst, UFormMain, USysBusiness,
  USysDB, UAdjustForm, UFormCard, UFormBase, UDataReport, UDataModule,
  NativeXml, IniFiles;
{$R *.dfm}
...
begin
  ...
  try
    ...
    begin
      ...
      FWebOrderItems[i].FProvID := nListB.Values['proid'];
...
var
  ...
  nMsg,nRecID, nUser,nPwd,nDBName,nServer, nConstr:string;
...
begin
  ...
  try
    ...
    nStr := 'select itemno,rcvprc,(rcvqty- isnull(pondqty,0)) as B_MaxValue,purpsn '+
            ' from rcvmst, rcvdet where rcvdet.srcvno=rcvmst.srcvno '+
            ' and rcvdet.srcvno=''%s'' and itemno=''%s''';
    nStr := Format(nStr,[nWebOrderItem.Fpurchasecontract_no,nWebOrderItem.FGoodsID]);
    ...
    begin
      ...
      FMaxQuantity := FieldByName('B_MaxValue').AsFloat;
      nWebOrderItem.FGoodPrc := FieldByName('rcvprc').AsString;
      nWebOrderItem.FSaleID := FieldByName('purpsn').AsString;
    end;
  finally
    nAdoQryJr.Free;
    nAdoJR.Free;
  ...
  nStr := Format(nStr,[sTable_Provider,nWebOrderItem.FProvID]);
  with fdm.QueryTemp(nStr) do
  begin
    if RecordCount<=0 then
    begin
      nMsg := '供应商[%s]不存在。';
      nMsg := Format(nMsg,[nWebOrderItem.FProvID]);
      ShowMsg(nMsg,sError);
      Writelog(nMsg);
      Exit;
    end;

    nWebOrderItem.FProvName := FieldByName('P_Name').AsString;
  end;

  nStr := 'Select * from %s where S_Id=''%s''';
  nStr := Format(nStr,[sTable_Salesman,nWebOrderItem.FSaleID]);
  with fdm.QueryTemp(nStr) do
  begin
    if RecordCount<=0 then
    begin
      nMsg := '业务员[%s]不存在。';
      nMsg := Format(nMsg,[nWebOrderItem.FSaleID]);
      ShowMsg(nMsg,sError);
      Writelog(nMsg);
      Exit;
    end;

    nWebOrderItem.FSaleName := FieldByName('S_Name').AsString;
  end;


  nStr := 'Select * from %s where M_ID=''%s''';
  nStr := Format(nStr,[sTable_Materails,nWebOrderItem.FGoodsID]);
  with FDM.QueryTemp(nStr) do
  begin
    if RecordCount<=0 then
    begin
    ...
    nWebOrderItem.FGoodsname := fieldbyname('M_Name').AsString;
...
begin
  ...
  try
    ...
    nList.Values['SaleID'] := nOrderItem.Fsaleid;
    nList.Values['SaleMan'] := nOrderItem.FSaleName; 
    nList.Values['StockPrc'] := nOrderItem.FGoodPrc;
